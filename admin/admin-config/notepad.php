...
$settings_qry = mysql_query("SELECT * FROM store_settings WHERE contenttype is null AND constant = 'NOTEPAD_TEXT' AND (hidden = '' OR hidden is null) ORDER BY name ASC;");
...
$field = $htmlfunctions->textarea('settings'.$settings_rslt['store_settings_id'],35,4,$field_value);
break;
}

$settings_list .= '<tr><td valign="top" colspan="2"><strong>'.$settings_rslt['name'].'</strong><br>'.$settings_rslt['description'].'<br>'.$field.'</td></tr>';
}


// submit changes
if ($form_check == 1) {
$settings_qry = mysql_query("SELECT store_settings_id FROM store_settings WHERE contenttype is null AND constant = 'NOTEPAD_TEXT' AND hidden is null ORDER BY name ASC;");
while ($settings_rslt = mysql_fetch_array($settings_qry)) {
...
$values = array(str_replace(array("\'",'\"'),array("'",'"'),$_POST[$field]),$settings_rslt['store_settings_id']);
$sth = $db->prepare($sql);
...
<title>Admin Config: Notepad</title>
...
<table width="800" border="0" align="center" cellpadding="3" class="main_table">
  <tr>
    ...
    <td valign="top">
	<div class="bc_nav"><?PHP echo brc(array('Global Settings' => 'admin-config/globalsettings.php')); ?></div>
