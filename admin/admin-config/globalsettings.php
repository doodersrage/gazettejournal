...
$settings_qry = mysql_query("SELECT * FROM store_settings WHERE " . (!empty($edit) ? "store_settings_id = '".$edit."' AND " : "") . "contenttype is null AND constant <> 'NOTEPAD_TEXT' and constant <> 'CONTACT_US_TEXT' AND (hidden = '' OR hidden is null) ORDER BY name ASC;");
...
$values = array($_POST['settings'.$edit],$edit);
$sth = $db->prepare($sql);
