...
$settings_qry = mysql_query("SELECT * FROM store_settings WHERE constant = 'CHURCH_NEWS_CONTENT' AND (hidden = '' OR hidden is null) ORDER BY name ASC;");
...
$field = $htmlfunctions->textarea('settings'.$settings_rslt['store_settings_id'],35,25,$field_value);
break;
}
...
$values = array(str_replace(array("\'",'\"'),array("'",'"'),$_POST['settings31']),31);
$sth = $db->prepare($sql);
...
<title>Admin Config: Church News Page</title>
...
<table width="800" border="0" align="center" cellpadding="3" class="main_table">
  <tr>
    ...
    <td valign="top">
	<div class="bc_nav"><?PHP echo brc(array('Church News' => 'admin-config/church-news.php')); ?></div>
